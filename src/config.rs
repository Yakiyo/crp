...
impl std::convert::From<Toml> for Config {
    fn from(v: Toml) -> Self {
		let l_img = if v.images.large_image.is_none() {
			...
			Some((v.images.large_image.unwrap(), v.images.large_image_tooltip.unwrap_or_default()))
		};
		let s_img = if v.images.small_image.is_none() {
			None
		} else {
			Some((v.images.small_image.unwrap(), v.images.small_image_tooltip.unwrap_or_default()))
		};
        Config {
            id: v.id,
            state: v.state.state,
